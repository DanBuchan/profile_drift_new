...
percentages <- aggregate(merged_data$percentage_correct, list(merged_data$type), mean)
colnames(percentages) <- c('type', 'mean_correct')
percentages$mean_drift <- tapply(merged_data$percentage_drift, list(merged_data$type), mean)
percentages$mean_nobest <-tapply(merged_data$percentage_nobest, list(merged_data$type), mean)

write.csv(percentages, "/home/dbuchan/Projects/profile_drift/results_data/drift/best_hits/hmm_drift_percentages.csv")
