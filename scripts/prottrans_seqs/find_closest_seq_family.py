...
# usage: python scripts/hmmer_seqs/find_closest_seq_family.py masked_25_percent_targets.fa ~/Data/pfam/Pfam-A.full.uniprot.fa output/ 10

def get_generated_sequences(hmm_seqs):
    seqs = []
...
def find_closest_fasta(all_family_seqs, gen_seqs, resultspath, i):
    ...
    match = re.search("^>.+\|(PF\d+_masked\d\d_\d+)", seq_record['header'])
    query_name = ''
    if match:
...
generated_seqs = get_generated_sequences(sys.argv[1])
# 51k generated seqs
